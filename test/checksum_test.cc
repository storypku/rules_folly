...
void testCRC32CContinuation(
  ...
  for (auto expected : expectedResults) {
    ...
    uint32_t result = impl(buffer + expected.offset + partialLength,
...
TEST(Checksum, crc32c_software) { testCRC32C(folly::detail::crc32c_sw); }
...
TEST(Checksum, crc32c_autodetect) { testCRC32C(folly::crc32c); }
...
TEST(Checksum, crc32_continuation) {
  if (folly::detail::crc32c_hw_supported()) {
    ...
    for (auto expected : expectedResults) {
      ...
      sw_res = folly::detail::crc32_sw(buffer + expected.offset + halflen,
                                       halflen, sw_res);
      uint32_t hw_res =
          folly::detail::crc32_hw(buffer + expected.offset, halflen, 0);
      hw_res = folly::detail::crc32_hw(buffer + expected.offset + halflen,
                                       halflen, hw_res);
      EXPECT_EQ(sw_res, hw_res);
      uint32_t sw_res2 =
