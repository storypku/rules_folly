...
void run_mt_sequencer_thread(int numThreads, int numOps, uint32_t init,
                             TurnSequencer<Atom>& seq,
                             Atom<uint32_t>& spinThreshold, int& prev, int i) {
...
void run_mt_sequencer_test(int numThreads, int numOps, uint32_t init) {
  ...
  for (int i = 0; i < numThreads; ++i) {
    threads[i] = DSched::thread(
        std::bind(run_mt_sequencer_thread<Atom>, numThreads, numOps, init,
                  std::ref(seq), std::ref(spinThreshold), std::ref(prev), i));
  }
...
void intrusive_ptr_add_ref(RefCounted const* p) { p->rc++; }
...
void runTryEnqDeqThread(int numThreads, int n, /*numOps*/
                        MPMCQueue<int, Atom, Dynamic>& cq,
                        std::atomic<uint64_t>& sum, int t) {
...
void runTryEnqDeqTest(int numThreads, int numOps) {
  ...
  for (int t = 0; t < numThreads; ++t) {
    threads[t] =
        DSched::thread(std::bind(runTryEnqDeqThread<Atom, Dynamic>, numThreads,
                                 n, std::ref(cq), std::ref(sum), t));
  }
  for (auto& t : threads) {
    DSched::join(t);
  }
...
template <typename Q, class Clock = steady_clock,
...
template <typename Q>
string producerConsumerBench(Q&& queue, string qName, int numProducers,
                             int numConsumers, int numOps,
  ...
  return folly::sformat(
      ...
      qName, numProducers, writer.methodName(), numConsumers, nanosPer, csw, n,
      failures, allocated);
}
...
void runMtProdConsDeterministicDynamic(long seed, uint32_t prods, uint32_t cons,
                                       uint32_t numOps, size_t cap,
                                       size_t minCap, size_t mult) {
  // we use the Bench method, but perf results are meaningless under DSched
  DSched sched(DSched::uniform(seed));
  ...
  for (const auto& caller : callers) {
    LOG(INFO) << producerConsumerBench(
        ...
        prods, cons, numOps, *caller);
  }
}

TEST(MPMCQueue, mt_prod_cons_deterministic) { runMtProdConsDeterministic(0); }
...
TEST(MPMCQueue, mt_prod_cons_deterministic_dynamic_with_arguments) {
  ...
  runMtProdConsDeterministicDynamic(seed, prods, cons, numOps, cap, minCap,
                                    mult);
}
...
TEST(MPMCQueue, mt_prod_cons) { runMtProdCons(); }

TEST(MPMCQueue, mt_prod_cons_dynamic) { runMtProdCons</* Dynamic = */ true>(); }
...
TEST(MPMCQueue, mt_prod_cons_emulated_futex) { runMtProdConsEmulatedFutex(); }
...
template <template <typename> class Atom, bool Dynamic = false>
void runNeverFailThread(int numThreads, int n, /*numOps*/
                        MPMCQueue<int, Atom, Dynamic>& cq,
                        std::atomic<uint64_t>& sum, int t) {
  uint64_t threadSum = 0;
...
uint64_t runNeverFailTest(int numThreads, int numOps) {
  ...
  for (int t = 0; t < numThreads; ++t) {
    threads[t] =
        DSched::thread(std::bind(runNeverFailThread<Atom, Dynamic>, numThreads,
                                 n, std::ref(cq), std::ref(sum), t));
  }
  for (auto& t : threads) {
    DSched::join(t);
...
void runNeverFailUntilThread(int numThreads, int n, /*numOps*/
                             MPMCQueue<int, Atom, Dynamic>& cq,
                             std::atomic<uint64_t>& sum, int t) {
  uint64_t threadSum = 0;
  for (int i = t; i < n; i += numThreads) {
...
uint64_t runNeverFailTest(int numThreads, int numOps) {
  ...
  for (int t = 0; t < numThreads; ++t) {
    threads[t] = DSched::thread(
        std::bind(runNeverFailUntilThread<Clock, Atom, Dynamic>, numThreads, n,
                  std::ref(cq), std::ref(sum), t));
  }
  for (auto& t : threads) {
    DSched::join(t);
  }
...
void runMtNeverFailUntilSystem(std::vector<int>& nts, int n) {
  for (int nt : nts) {
    uint64_t elapsed =
        runNeverFailTest<std::chrono::system_clock, std::atomic, Dynamic>(nt,
                                                                          n);
    LOG(INFO) << (elapsed * 1000.0) / (n * 2) << " nanos per op with " << nt
              << " threads";
  }
...
void runMtNeverFailUntilSteady(std::vector<int>& nts, int n) {
  for (int nt : nts) {
    uint64_t elapsed =
        runNeverFailTest<std::chrono::steady_clock, std::atomic, Dynamic>(nt,
                                                                          n);
    LOG(INFO) << (elapsed * 1000.0) / (n * 2) << " nanos per op with " << nt
              << " threads";
  }
...
TEST(MPMCQueue, queue_moving) { run_queue_moving(); }

TEST(MPMCQueue, queue_moving_dynamic) { run_queue_moving<true>(); }
...
TEST(MPMCQueue, try_read_until) { testTryReadUntil<false>(); }

TEST(MPMCQueue, try_read_until_dynamic) { testTryReadUntil<true>(); }

TEST(MPMCQueue, try_write_until) { testTryWriteUntil<false>(); }

TEST(MPMCQueue, try_write_until_dynamic) { testTryWriteUntil<true>(); }
