...
// https://github.com/facebook/folly/blob/v2021.09.06.00/folly/test/small_vector_test.cpp
...
static_assert(sizeof(small_vector<int32_t, 1, uint32_t>) == 8 + 4,
              "small_vector<int32_t,1,uint32_t> is wrong size");

// Extra 2 bytes needed for alignment.
static_assert(sizeof(small_vector<int32_t, 1, uint16_t>) == 8 + 2 + 2,
              "small_vector<int32_t,1,uint16_t> is wrong size");
static_assert(alignof(small_vector<int32_t, 1, uint16_t>) >= 4,
              "small_vector not aligned correctly");

// Extra 3 bytes needed for alignment.
static_assert(sizeof(small_vector<int32_t, 1, uint8_t>) == 8 + 1 + 3,
              "small_vector<int32_t,1,uint8_t> is wrong size");
static_assert(alignof(small_vector<int32_t, 1, uint8_t>) >= 4,
              "small_vector not aligned correctly");
...
static_assert(!folly::is_trivially_copyable<std::unique_ptr<int>>::value,
              "std::unique_ptr<> is trivially copyable");

static_assert(alignof(small_vector<std::aligned_storage<32, 32>::type, 4>) ==
                  32,
              "small_vector not aligned correctly");
...
using small_sorted_vector_map =
    folly::sorted_vector_map<Key, Value, std::less<Key>,
                             std::allocator<std::pair<Key, Value>>, void,
                             folly::small_vector<std::pair<Key, Value>, N>>;

template <typename Key, typename Value, size_t N>
using noheap_sorted_vector_map = folly::sorted_vector_map<
    Key, Value, std::less<Key>, std::allocator<std::pair<Key, Value>>, void,
    folly::small_vector<std::pair<Key, Value>, N,
...
using noheap_sorted_vector_set = folly::sorted_vector_set<
    T, std::less<T>, std::allocator<T>, void,
...
static_assert(!folly::is_trivially_copyable<NontrivialType>::value,
...
static_assert(!folly::is_trivially_copyable<NoncopyableCounter>::value,
...
TEST(small_vector, BasicGuarantee) {
  for (int prepop = 1; prepop < 30; ++prepop) {
    (TestBasicGuarantee(prepop))(  // parens or a mildly vexing parse :(
        1, [&](folly::small_vector<Thrower, 3>& v) { v.emplace_back(); });
...
TEST(small_vector, NoHeap) {
  typedef folly::small_vector<std::string, 10, std::size_t,
                              folly::small_vector_policy::NoHeap>
      Vector;
